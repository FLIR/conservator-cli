...
def read_config(args, default_config_path: str, show_help: Callable) -> str:
   ...
   example 2:
    ...
    if args.config is None:
        print(f'ERROR: Config not provided... This is the default config path that was provided: {default_config_root_path}')
    ...
    if not osp.exists(config_path):
        ...
        config_path = osp.join(default_config_root_path, args.config)  # Try it as a filename

        if not osp.exists(config_path):
            config_path = osp.join(default_config_root_path, f'{args.config}.json')  # Try to put a json extension
...
def read_subfolder_config(args, default_config_path: str, config_filename: str, show_help: Callable) -> str:
    """
    Parse config from input arguments. Otherwise call show_help(..)
