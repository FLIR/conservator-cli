...
class Stats:
    ...
    def __str__(self):
        ...
        t.align = 'l'
        t.add_row(["Total entries",
                  self.upload_entry_count])
        t.add_row(["Total entries commented out by user",
                  self.upload_entry_commented_out_count])
        t.add_row(["Total entries that are invalid",
                  self.upload_entry_invalid_count])
        t.add_row(["Videos would be uploaded",
                  self.would_upload_video_count])
        t.add_row(["Videos ACTUALLY uploaded",
                  self.upload_video_count])
        t.add_row(["Videos currently being processed by Conservator",
                  self.video_still_processing_count])
        t.add_row(["Video metadata updated",
                  self.upload_metadata_count])
...
def _update_custom_video_helper(video_metadata: dict, existing_custom: dict, upload_custom: dict) -> bool:
    ...
    if existing_video_source is None:
        ...
        # if json.dumps(existing_video_source, indent=0, sort_keys=True) != json.dumps(video_source, indent=0, sort_keys=True):
        if existing_video_source.get('source_name') != upload_video_source.get('source_name'):
            video_metadata['custom']['video_source'] = upload_video_source
            has_updates = True
        # Otherwise video source is the same...

    # --------------------------------
    #         Update RGBT
    # --------------------------------
    existing_destination = existing_custom.get(
        'rgbt', {}).get('rectify', {}).get('destination')
    existing_apply_undistort = existing_custom.get(
        'rgbt', {}).get('rectify', {}).get('apply_undistort')
    upload_rgbt = upload_custom.get('rgbt')
    if type(existing_destination) is not dict:
        ...
        if upload_rgbt['rectify']['destination']['point_tl'] != existing_destination['point_tl'] or \
                upload_rgbt['rectify']['destination']['point_tr'] != existing_destination['point_tr'] or \
                upload_rgbt['rectify']['destination']['point_bl'] != existing_destination['point_bl'] or \
                upload_rgbt['rectify']['destination']['point_br'] != existing_destination['point_br'] or \
                upload_rgbt['rectify'].get('apply_undistort') != existing_apply_undistort:
...
def update_video_meta_data(
    ...
    if type(override_location) is str and len(override_location) > 0:
    ...
    if type(upload_custom) is dict:
        ...
        if existing_custom is None:
            ...
            # Selective update
            if _update_custom_video_helper(video_metadata, existing_custom, upload_custom):
                has_updates = True
                _logger.debug(
                    '       Metadata needs to be updated because of: custom')
    ...
    if has_updates:
        with open(save_path, 'w') as f:
            json.dump(video_metadata_all, f, sort_keys=False,
                      indent=1, separators=(",", ": "))

        if dry_run is False:
            video.upload_metadata(save_path)
            stats.upload_metadata_count += 1
            _logger.info(f'       Successfully updated video metadata')
    else:
        if dry_run is False:
            _logger.info(f'       Video does not require metadata updates')
...
def upload_video_helper(local_path: str, remote_name: str, collection: Collection):
    ...
    # You can wait for processing in this manner (not required). This script is designed to run multiples times
...
def get_video_and_collection(filename: str, conservator_location: str):
    ...
    if collection is not None:
        for video in collection.recursively_get_videos(fields=["name", "owner"], search_text=f'name:"{filename}"'):
            video: Video
...
def upload_video(full_file_path: str, filename: str, collection: Collection, conservator_location: str) -> bool:
    """
...
def check_if_expanded_properly(path: str) -> None:
    ...
    for p in path_parts:
        ...
        if p[0] == '$':
            raise Exception(
                f'Error: could not expand environment variable {p}. Do you forget to set it? Try: export {p}=some_path')


def still_processed_message():
    _logger.info(f'       Video is still being processed by Conservator')


def upload_video_capture(row: dict, default_metadata: DefaultMetadata, stats: Stats, dry_run: bool, camera_name_or_spectrum: str):
    """
    :param row:
    ...
    if not osp.isfile(local_path):
        raise Exception(
            f'For a video upload the "local_path" is expected to be a file. The following is not a file: "{local_path}"')
    ...
    if video is None:
        ...
        if dry_run is False:
            ...
            upload_video(local_path, filename, collection=collection,
        ...
        if _conservator.videos.is_uploaded_media_id_processed(video.id):
            if dry_run is False:
                ...
                _logger.info(
                    f'       Downloading video metadata to temporary file: {meta_data_path}')
...
def upload_prism_capture(row: dict, default_metadata: DefaultMetadata, stats: Stats, dry_run: bool):
    """
    :param row:
    :param default_metadata:
    :param stats: keep track of stats to display to user
    ...
    if not osp.exists(thermal_input_path) or not osp.isdir(thermal_input_path):
        stats.upload_entry_invalid_count += 1
        _logger.warning(
            f'The path "{thermal_input_path}" is not a directory or does not exist (skipping entry). Please make sure this is a valid Prism recording...')
        return

    if not osp.exists(thermal_input_path) or not osp.isdir(thermal_input_path):
        stats.upload_entry_invalid_count += 1
        _logger.warning(
            f'The path "{visible_input_path}" is not a directory or does not exist (skipping entry). Please make sure this is a valid Prism recording...')
        return
    ...
    try:
        meta = lib.prism_capture.get_first_meta(thermal_meta_path)
    except Exception as e:
        stats.upload_entry_invalid_count += 1
        print(e)
        _logger.warning(
            f'Could not read the first thermal meta file at "{thermal_meta_path}" (skipping entry). Please make sure this is a valid Prism recording...')
        return
    ...
    if thermal_video is None:
        ...
        if dry_run:
            _logger.info(
                f'Prism AI thermal exists and is ready for upload: {thermal_input_path}')
        ...
        if not osp.exists(thermal_zip_path):
            ...
            _logger.info(
                f'     Creating zip archive for upload: {upload_root_path}/{thermal_zip_filename}')
        ...
        if not dry_run:
            ...
            upload_video(thermal_zip_path, thermal_zip_filename,
                         collection, conservator_location)
            stats.upload_video_count += 1
    else:
        thermal_video: Video
        if _conservator.videos.is_uploaded_media_id_processed(thermal_video.id):
            ...
            if dry_run is False:
                ...
                _logger.info(
                    f'       Downloading video metadata to temporary file: {thermal_meta_data_path}')

                with open(thermal_meta_data_path, 'r') as f:
                    thermal_video_metadata = json.load(f)
        else:
            stats.video_still_processing_count += 1
            still_processed_message()
    ...
    if visible_video is None:
        ...
        if dry_run:
            _logger.info(
                f'Prism AI visible exists and is ready for upload: {visible_input_path}')
        ...
        if not osp.exists(visible_zip_path):
            ...
            _logger.info(
                f'     Creating zip archive for upload: {upload_root_path}/{visible_zip_filename}')
            _logger.debug('     Zip command:')
            _logger.debug('         ' + cmd)
            os.system(cmd)
        ...
        if not dry_run:
            ...
            upload_video(visible_zip_path, visible_zip_filename,
                         collection, conservator_location)
            stats.upload_video_count += 1
    else:
        visible_video: Video
        if _conservator.videos.is_uploaded_media_id_processed(visible_video.id):
            ...
            if dry_run is False:
                ...
                _logger.info(
                    f'       Downloading video metadata to temporary file: {visible_meta_data_path}')

                with open(visible_meta_data_path, 'r') as f:
                    visible_video_metadata = json.load(f)
    ...
    if thermal_video_metadata is not None and visible_video_metadata is not None:
        if len(thermal_video_metadata["videos"][0]['frames']) == len(visible_video_metadata["videos"][0]['frames']):
            for i, thermal_frame in enumerate(thermal_video_metadata["videos"][0]['frames']):
...
def main():
    ...
    parser.add_argument(
        '--config_root', help='Override where upload configurations are read (defaults to $PWD/config/upload)')
    # False by default...
    parser.add_argument(
    ...
    if args.config_root and type(args.config_root) is str and len(args.config_root) > 0:
    ...
    config_path = lib.config_reader.read_subfolder_config(
    ...
    with open(upload_list_path) as f:
        ...
        for row in reader:
            ...
            default_metadata: DefaultMetadata = default_metadata_dict.get(
                hardware_name)
            ...
            if upload_type == 'prism' or upload_type == 'guardian':  # For backwards compatibility
                upload_prism_capture(row, default_metadata, stats, dry_run)
            elif upload_type == 'video':
                upload_video_capture(row, default_metadata,
                                     stats, dry_run, camera_name_or_spectrum)
            else:
                _logger.warning(
