...
class DatasetCache:
    ...
    def _pull_checkout_dataset(self, id, name, dataset_version):
        ...
        if not os.path.exists(name):
            subp = subprocess.call(
                ["git", "clone", "https://{}@flirconservator.com/git/dataset_{}".format(email, id), "{}".format(name)],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            os.chdir(name)
            subp = subprocess.call([python_command, "./cvc.py", "remote", "add",
                                    "https://{}:{}@flirconservator.com/dvc".format(email, self.credentials.token)],
                                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            os.chdir("..")
        ...
        subp = subprocess.call(["git", "checkout", dataset_version], stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL)
        subp = subprocess.call([python_command, "./cvc.py", "pull"], stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL)
        subp = subprocess.call([python_command, "./cvc.py", "pull"], stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL)
        os.chdir(save)
    ...
    def get_index_filepath(self, dataset_name, dataset_version):
        ...
        subp = subprocess.call(["cp", os.path.join(self.cache_path, dataset_name, "index.json"), cached_index_path],
